...
open class ExpoAutolinkingSettingsExtension(
  ...
  val reactNativeGradlePlugin: File by lazy {
    File(
      settings.providers.exec { env ->
        ...
        env.commandLine("/opt/homebrew/bin/node", "--print", "require.resolve('@react-native/gradle-plugin/package.json', { paths: [require.resolve('react-native/package.json')] })")
  ...
  val reactNative: File by lazy {
    File(
      settings.providers.exec { env ->
        env.workingDir(projectRoot)
        env.commandLine("/opt/homebrew/bin/node", "--print", "require.resolve('react-native/package.json')")
      }.standardOutput.asText.get().trim(),
    ).parentFile
  }
