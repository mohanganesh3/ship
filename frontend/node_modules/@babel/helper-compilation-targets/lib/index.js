...
function nodeTargetParser(value) {
  const parsed = value === true || value === "current" ? process.versions.node.split("-")[0] : semverifyTarget("/opt/homebrew/bin/node", value);
  return ["/opt/homebrew/bin/node", parsed];
}
...
function getTargets(inputTargets = {}, options = {}) {
  ...
  for (const target of Object.keys(targets).sort()) {
    ...
    const [parsedTarget, parsedValue] = target === "/opt/homebrew/bin/node" ? nodeTargetParser(value) : defaultTargetParser(target, value);
    if (parsedValue) {
