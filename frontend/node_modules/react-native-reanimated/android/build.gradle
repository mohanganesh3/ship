...
def resolveReactNativeWorkletsDirectory() {
    ...
    def reactNativeWorkletsPackage = file(
        providers.exec {
            workingDir(rootDir)
            commandLine("/opt/homebrew/bin/node", "--print", "require.resolve('react-native-worklets/package.json')")
        }.standardOutput.asText.get().trim()
    )
...
def validateReactNativeVersionResult = providers.exec {
    ...
    commandLine("/opt/homebrew/bin/node", "./../scripts/validate-react-native-version.js", REACT_NATIVE_VERSION.toString())
