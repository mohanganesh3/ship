...
class TeacherNotReadyError(RuntimeError):
    """Raised when the local teacher server is up but not yet ready (e.g., 503 Loading model)."""


class TeacherRequestError(RuntimeError):
    """Raised when a teacher request fails after retries."""


def is_teacher_ready(port: int, timeout_s: float = 2.0) -> bool:
    """Return True if the teacher port responds with HTTP 200 for a lightweight endpoint."""
    # llama-server exposes /health and /v1/models, but both return 503 while loading.
    # Use GET /health because it is cheap and doesn't require JSON.
...
def wait_for_teachers_ready(
    ports: Sequence[int],
    poll_secs: int,
    startup_grace_secs: int,
    stop_event: threading.Event,
) -> None:
    """Block until all teacher ports are ready (HTTP 200 on /health)."""
    poll_secs = max(1, int(poll_secs))
    startup_grace_secs = max(0, int(startup_grace_secs))

    if startup_grace_secs:
        # Give servers time to bind their ports before we begin probing.
        for _ in range(startup_grace_secs):
            if stop_event.is_set():
                return
            time.sleep(1)

    while not stop_event.is_set():
        not_ready = [p for p in ports if not is_teacher_ready(p)]
        if not not_ready:
            return
        log_pipe(f"generate_wave1.py: waiting for teacher readiness ports_not_ready={not_ready}")
...
def build_modeA_tasks(chunk: Chunk) -> List[Tuple[str, str]]:
    ...
    common = (
        ...
        "If the excerpt lacks a detail needed to answer, state 'Not specified in the excerpt' for that detail. "
        "Reply 'INSUFFICIENT_SOURCE' only if the excerpt is totally unrelated to the task or provides no usable information. "
...
def build_modeB_task(chunk_a: Chunk, chunk_b: Chunk) -> Tuple[str, str]:
    instruction = (
        ...
        "Do not use outside knowledge. "
        "If an excerpt lacks a detail needed to answer, state 'Not specified in the excerpt' for that detail. "
        "Reply 'INSUFFICIENT_SOURCE' only if the excerpts are totally unrelated to the task or provide no usable information. "
        "Do not mention being an AI or a language model.\n\n"
...
def call_teacher(
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            # teacher model to respond. Use the configured timeout to avoid client aborts.
            resp = requests.post(url, json=payload, timeout=timeout)
            if resp.status_code in {502, 503, 504}:
                # llama-server returns a structured OpenAI-style error body while loading.
                raise TeacherNotReadyError(f"http_{resp.status_code}:{resp.text[:200]}")
            resp.raise_for_status()
            data = resp.json()
            return str(data.get("choices", [{}])[0].get("message", {}).get("content", ""))
        except TeacherNotReadyError as e:
            last_err = e
    ...
    # Preserve TeacherNotReadyError type so callers can decide to requeue/backoff.
    if isinstance(last_err, TeacherNotReadyError):
        raise last_err
    raise TeacherRequestError(f"teacher_call_failed_after_retries: {last_err}")
...
class PortWorker(threading.Thread):
    ...
    def run(self) -> None:
        ...
        # Backoff used when teacher ports are loading or temporarily unavailable.
        # Start with a longer delay to avoid hammering llama-server while it is loading a
        # huge model (which can materially slow down load due to extra logging/HTTP work).
        not_ready_backoff = 30
        while not self.stop_event.is_set():
            ...
            for attempt in range(1, self.args.sample_retries + 1):
                try:
                    ...
                    # Got a response from the teacher: reset not-ready backoff.
                    not_ready_backoff = 30
                except TeacherNotReadyError as e:
                    # Teacher is still loading. Don't write empties. Requeue and back off.
                    last_issues = [f"teacher_not_ready({e})"]
                    time.sleep(not_ready_backoff)
                    not_ready_backoff = min(300, not_ready_backoff + 30)
                    # Requeue the job so the run doesn't terminate while teachers are loading.
                    try:
                        self.q.put(job, timeout=5)
                    except Exception:
                        # If the queue is full, just try again later.
                        pass
                    self.q.task_done()
                    requeued_not_ready = True
                    break
                except Exception as e:
            ...
            # If we broke out early due to not-ready requeue, move on.
            if requeued_not_ready or self.stop_event.is_set():
                continue

            # If the teacher produced an empty response (including total request failure),
            # do NOT write a record and do NOT mark progress done. This keeps the job
            # retryable without filling the output with empty lines.
            if not (isinstance(content, str) and content.strip()):
                # For non-ready cases we already requeued above. For other repeated failures,
                # requeue a limited number of times, then give up to avoid infinite loops.
                job.failures += 1
                if job.failures <= 10 and not self.stop_event.is_set():
                    try:
                        self.q.put(job, timeout=5)
                    except Exception:
                        pass
                    ...
                    log_pipe(
                        f"generate_wave1.py: dropping job after repeated empty failures key={key} port={self.port} failures={job.failures} issues={last_issues}"
                    )
                self.q.task_done()
                continue
...
class ProgressStore:
    ...
    def discard(self, key: str) -> None:
        """Remove a key from the done-set (used for repair/purge flows)."""
        with self._lock:
...
def main() -> int:
    ...
    ap.add_argument("--request-timeout", type=int, default=360)
    # Requirement: retry failed requests 3 times, then skip (do not crash).
    ap.add_argument("--request-retries", type=int, default=3)
    # Retry a sample if the teacher returns empty/invalid output.
    ap.add_argument("--sample-retries", type=int, default=3)
    ap.add_argument(
        "--purge-empty-progress",
        action="store_true",
        help=(
            "Scan existing wave1_mode*.jsonl outputs for records with empty responses and "
            "remove their keys from generation_progress.json so they will be regenerated on resume. "
            "Does not rewrite the output JSONL files (new regenerated records are appended)."
        ),
    )
    ap.add_argument(
        ...
        action="store_true",
        help=(
            "Scan existing wave1_mode*.jsonl outputs for records whose response is exactly 'INSUFFICIENT_SOURCE' "
            "and remove their keys from generation_progress.json so they will be regenerated on resume. "
            "Does not rewrite the output JSONL files (new regenerated records are appended)."
        ),
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        action="store_true",
        help=(
            "Before generating, wait until all teacher ports return HTTP 200 on /health. "
            "This avoids spamming the server with 503 requests while it is still loading the model."
        ),
    )
    ap.add_argument(
        "--teacher-ready-poll-secs",
        type=int,
        default=30,
        help="Polling interval (seconds) for --wait-for-teachers readiness checks",
    )
    ap.add_argument(
        "--teacher-startup-grace-secs",
        type=int,
        default=5,
        help="Initial grace period (seconds) before readiness polling begins",
    )
    ...
    if args.wait_for_teachers:
        wait_for_teachers_ready(
            ports=ports,
            poll_secs=args.teacher_ready_poll_secs,
            startup_grace_secs=args.teacher_startup_grace_secs,
            stop_event=stop_event,
        )

    if args.purge_empty_progress or args.purge_insufficient_progress:
        purged = 0
        scanned = 0
        scan_modes = [m for m in modes if out_files[m].exists()]
        for m in scan_modes:
            try:
                with open(out_files[m], "r", encoding="utf-8") as f:
                    for line in f:
                        line = line.strip()
                        if not line:
                            continue
                        ...
                        is_empty = (not isinstance(resp, str)) or (not resp.strip())
                        is_insufficient = isinstance(resp, str) and resp.strip() == "INSUFFICIENT_SOURCE"
                        if (args.purge_empty_progress and is_empty) or (
                            args.purge_insufficient_progress and is_insufficient
                        ):
                            mode = str(rec.get("mode") or m)
                            angle = str(rec.get("angle") or "")
                            chunk_ids = rec.get("chunk_ids") or []
                            if not isinstance(chunk_ids, list):
                                chunk_ids = []
                            key = progress.make_key(mode, tuple(str(x) for x in chunk_ids), angle)
                            if progress.is_done(key):
                                progress.discard(key)
                                purged += 1
            except Exception as e:
                log_pipe(f"generate_wave1.py: purge_progress scan failed for mode={m}: {e}")
        ...
        log_pipe(
            "generate_wave1.py: purge_progress complete "
            + f"purge_empty={bool(args.purge_empty_progress)} "
            + f"purge_insufficient={bool(args.purge_insufficient_progress)} "
            + f"purged={purged} scanned={scanned}"
        )
