...
nonempty_lines() {
  ...
  # For Wave1 raw outputs, count only records with a non-empty `response`.
  # These files can contain many JSONL rows with "response": "" from earlier teacher-not-ready windows,
  # which would otherwise inflate progress counters and prematurely trigger SuperFilter.
  if [[ "$f" == *"wave1_mode"*_raw.jsonl ]]; then
    local total empty
    total=$(grep -cve '^\s*$' "$f" 2>/dev/null || echo 0)
    empty=$(grep -cE '"response"[[:space:]]*:[[:space:]]*""' "$f" 2>/dev/null || echo 0)
    if [[ "$total" =~ ^[0-9]+$ ]] && [[ "$empty" =~ ^[0-9]+$ ]] && [[ "$total" -ge "$empty" ]]; then
      echo $((total - empty))
      return
    fi
  fi
...
start_superfilter() {
  ...
  if [[ "$attempt" == "1" ]]; then
    ...
    # Last-resort: skip IFD entirely and just use hard rejection, widened.
    # This is intentionally conservative: we only do it after multiple IFD attempts
    # failed to reach the minimum dataset size gate for SFT.
    args+=("--hard-only" "--widen-hard")
  fi
