#!/usr/bin/env python3
"""Phase 5 — Quantize student-1.7B to GGUF and write routing policy.
...
- Writes deterministic routing policy to deploy/routing_policy.py
...
# Threading (conversion/merge can be heavy)
...
ORPO_CHECKPOINT = Path("/home/mohanganesh/ship/training/checkpoints/orpo-1.7b/final")
MERGED_DIR = Path("/home/mohanganesh/ship/training/merged/maritime-1.7b-merged")
...
def write_routing_policy(dest: Path) -> None:
    dest.parent.mkdir(parents=True, exist_ok=True)
    code = '''#!/usr/bin/env python3
"""TTC Routing Policy — deterministic /think vs /no_think router"""

import re

THINK_KEYWORDS = [
    "diagnose", "troubleshoot", "why is", "why does", "why won't", "why can't",
    "calculate", "stability", "what would happen", "root cause", "failure mode",
    "step by step", "sequence of", "procedure for emergency", "exception to",
    "notwithstanding", "won't start", "doesn't start", "won't open", "not working",
    "acting up", "going wrong",
]

THINK_REGEX = [
    r"\\b\\d+\\s*(bar|kPa|rpm|kW|hp|nm|knots|GRT|DWT|ppm|m/m|percent)\\b",
    r"regulation\\s*\\d+",
    r"annex\\s+(i|ii|iii|iv|v|vi)\\b",
    r"chapter\\s+[ivxIVX]+\\b",
    r"rule\\s+\\d+",
]


def route(query: str) -> str:
    """Return '/think' or '/no_think' for a query."""
    q_lower = query.lower()
    ...
    for pattern in THINK_REGEX:
        if re.search(pattern, query, flags=re.IGNORECASE):
            return "/think"

    clause_words = ["and", "but", "however", "if", "unless", "except", "provided that", "notwithstanding"]
    clause_count = sum(q_lower.count(cw) for cw in clause_words)
    if clause_count >= 3:
        return "/think"
...
if __name__ == "__main__":
    tests = [
        "What does MARPOL Annex VI regulate?",
        "Our main engine won't start, what do I check?",
        "Calculate the GM given KG=8.5m and BM=1.2m",
        "What is the definition of ballast water?",
        "Why is the exhaust temperature on cylinder 4 rising and what should I do?",
        "What documents do I need for port state control?",
    ]

    for t in tests:
        print(f"{route(t):10s} | {t}")
'''
    dest.write_text(code, encoding="utf-8")


def main() -> None:
    phase = "PHASE_5_QUANTIZE_1.7B"
    ...
    del base
    gc.collect()
    log_pipeline(f"PHASE_5_QUANTIZE_1.7B STATUS: MERGE_LORA_DONE merged_dir={MERGED_DIR}")

    # Step 2: convert to GGUF f16
    f16_gguf = DEPLOY_DIR / "maritime-1.7b-f16.gguf"
    log_pipeline(f"PHASE_5_QUANTIZE_1.7B STATUS: GGUF_F16_CONVERT_START out={f16_gguf}")
    r = subprocess.run(
        [
    ...
    if r.returncode != 0:
        log_pipeline("PHASE_5_QUANTIZE_1.7B STATUS: FAIL — f16 GGUF conversion failed")
        sys.stderr.write(r.stderr)
        raise RuntimeError("convert_hf_to_gguf.py failed")

    # Step 3: quantize Q4_K_M
    q4km = DEPLOY_DIR / "maritime-1.7b-q4km.gguf"
    log_pipeline(f"PHASE_5_QUANTIZE_1.7B STATUS: QUANTIZE_Q4KM_START out={q4km}")
    r = subprocess.run([str(bins.quantize_bin), str(f16_gguf), str(q4km), "Q4_K_M"], capture_output=True, text=True)
    if r.returncode != 0:
        log_pipeline("PHASE_5_QUANTIZE_1.7B STATUS: FAIL — Q4_K_M quantization failed")
        sys.stderr.write(r.stderr)
        raise RuntimeError("llama-quantize failed")

    # Step 4: modal precision quick-check via llama-server
    log_pipeline("PHASE_5_QUANTIZE_1.7B STATUS: POST_QUANT_VALIDATION_START")
    port = 9001
    ...
    try:
        if not wait_for_health(f"http://127.0.0.1:{port}/health", timeout_s=300):
            log_pipeline("PHASE_5_QUANTIZE_1.7B STATUS: FAIL — Q4_K_M server failed to start")
        ...
        if not reg:
            log_pipeline("PHASE_5_QUANTIZE_1.7B STATUS: WARNING — no regulatory records found for validation")
        else:
            import random
            ...
            log_pipeline(f"PHASE_5_QUANTIZE_1.7B STATUS: Q4KM_MODAL_PRECISION {pct:.1f}%")

            if pct < 85:
                log_pipeline(f"PHASE_5_QUANTIZE_1.7B STATUS: Q4KM_TOO_LOW fallback=Q5_K_M")
                q5km = DEPLOY_DIR / "maritime-1.7b-q5km.gguf"
                r = subprocess.run([str(bins.quantize_bin), str(f16_gguf), str(q5km), "Q5_K_M"], capture_output=True, text=True)
                if r.returncode != 0:
                    log_pipeline("PHASE_5_QUANTIZE_1.7B STATUS: FAIL — Q5_K_M fallback quantization failed")
                    sys.stderr.write(r.stderr)
                    raise RuntimeError("Q5_K_M fallback failed")
                log_pipeline(f"PHASE_5_QUANTIZE_1.7B STATUS: COMPLETE fallback_model={q5km}")
            else:
                log_pipeline(f"PHASE_5_QUANTIZE_1.7B STATUS: COMPLETE model={q4km}")
    ...
    write_routing_policy(Path("/home/mohanganesh/ship/deploy/routing_policy.py"))
    log_pipeline("PHASE_5_QUANTIZE_1.7B STATUS: ROUTING_POLICY_WRITTEN deploy/routing_policy.py")
