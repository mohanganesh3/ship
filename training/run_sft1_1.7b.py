...
TAPT_CHECKPOINT_ROOT = Path("/home/mohanganesh/ship/training/checkpoints/tapt-1.7b")
...
def resolve_best_checkpoint_dir(phase_tag: str) -> tuple[Path, str]:
    """Prefer TAPT checkpoint if present, otherwise fall back to CPT checkpoint.
    Returns (checkpoint_path, checkpoint_type) where type is 'TAPT' or 'CPT'."""
    
    # First check for TAPT checkpoint
    tapt_final = TAPT_CHECKPOINT_ROOT / "final"
    if tapt_final.exists() and (tapt_final / "adapter_model.safetensors").exists():
        return tapt_final, "TAPT"
    
    # Check for any TAPT checkpoint
    if TAPT_CHECKPOINT_ROOT.exists():
        for p in sorted(TAPT_CHECKPOINT_ROOT.glob("checkpoint-*"), 
                       key=lambda x: int(x.name.split("-")[1]), reverse=True):
            if p.is_dir() and (p / "adapter_model.safetensors").exists():
                return p, "TAPT"
    
    # Fall back to CPT checkpoint
    final_dir = CPT_CHECKPOINT_ROOT / "final"
    if final_dir.exists():
        return final_dir, "CPT"
...
def load_think_examples():
    """Load /think-style examples from either curated or generated JSONL."""
    records = []
    with open(SFT_DATA) as f:
        for line in f:
            ...
            answer = rec.get("a", rec.get("answer", rec.get("response", "")))
            has_think_block = "<think>" in answer and "</think>" in answer
            if mode == "think" or has_thinking or has_think_block or qtype in ("troubleshooting", "calculation", "diagnostic"):
                records.append(rec)
        ...
        with open(SFT_DATA) as f:
            for line in f:
                ...
                try:
                    ...
                    continue
                ans = rec.get("a", rec.get("answer", rec.get("response", "")))
                if ("step" in ans.lower() or "<think>" in ans.lower()) and rec not in records:
                    records.append(rec)
...
def main() -> None:
    ...
    # Load checkpoint (prefer TAPT if available, otherwise CPT)
    ckpt_dir, ckpt_type = resolve_best_checkpoint_dir(phase)
    logger.info(f"Loading {ckpt_type} LoRA from {ckpt_dir}")
    base_model = PeftModel.from_pretrained(base_model, str(ckpt_dir))
    base_model = base_model.merge_and_unload()
    logger.info(f"{ckpt_type} LoRA merged into base weights.")
    ...
    log_pipeline("PHASE_2A_SFT1_1.7B STATUS: TRAINING STARTED")
    trainer.train()

    trainer.save_model(str(SFT1_CHECKPOINT / "final"))
    tokenizer.save_pretrained(str(SFT1_CHECKPOINT / "final"))

    log_pipeline("PHASE_2A_SFT1_1.7B STATUS: COMPLETE. Now run gate check.")
    ...
    if tested == 0:
        ...
        log_pipeline("PHASE_2A_SFT1_1.7B GATE: FAIL — no examples tested")
        sys.exit(1)
    ...
    if gate_pass:
        log_pipeline(f"PHASE_2A_SFT1_1.7B GATE: PASS ({pct:.1f}% reasoning traces)")
    else:
        log_pipeline(f"PHASE_2A_SFT1_1.7B GATE: FAIL ({pct:.1f}% < 70%). Need more training.")
        sys.exit(1)
