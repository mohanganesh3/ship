#!/usr/bin/env python3
"""training/run_cpt_4b.py

CPT for Qwen3-4B — single-GPU fp16 LoRA r=128 (K80-compatible)
...
PERPLEXITY_LOG = Path("/home/mohanganesh/ship/logs/cpt_perplexity_4b.jsonl")
CHECKPOINT_DIR = Path("/home/mohanganesh/ship/training/checkpoints/cpt-4b")
...
def latest_checkpoint_dir(root: Path) -> Path | None:
    """Return latest `checkpoint-<step>` directory under root, if any."""
    try:
        if not root.exists():
            return None
        best_step = -1
...
def _sanitize_checkpoint_for_transformers_resume(checkpoint_dir: Path, *, phase_tag: str) -> None:
    """Disable incompatible RNG-state files so resume won't crash.

    Some recent Transformers versions load RNG state via:
        torch.load(rng_state.pth, weights_only=True)
    which can fail if the saved RNG contains numpy objects.

    Our checkpoints are trusted (self-produced), so we simply disable RNG-state
    restore by renaming rng_state*.pth; Trainer will then skip loading it.
    """
    ...
    for rng_path in rng_files:
        try:
            torch.load(rng_path, weights_only=True)
            continue
        except TypeError:
            # Older torch without `weights_only` arg.
            return
        except Exception as e:
            disabled = rng_path.with_name(rng_path.name + ".disabled")
            try:
                rng_path.rename(disabled)
                log_pipeline(
                    f"{phase_tag} STATUS: RESUME_SANITIZE disabled {rng_path.name} "
                    f"(torch.load(weights_only=True) failed: {type(e).__name__})"
                )
            except Exception as e2:
                log_pipeline(
                    f"{phase_tag} WARNING — RESUME_SANITIZE could not disable {rng_path.name}: "
...
def build_packed_bin_cache(
    *,
    ...
    log_pipeline(f"PHASE_1A_CPT_4B STATUS: CACHE_BUILD_START ({log_prefix}) src={src_jsonl}")
    ...
    log_pipeline(f"PHASE_1A_CPT_4B STATUS: CACHE_BUILD_DONE ({log_prefix}) sequences={n_sequences} docs={n_docs}")
    return meta
...
class CurriculumPackedIterableDataset(IterableDataset):
    """Curriculum mixer over two packed-sequence memmaps (CPU/RAM safe)."""
    ...
    def set_stage(self, stage: int):
        if stage != self.current_stage:
            ...
            logger.info(f"Curriculum: stage={stage} maritime={m*100:.0f}% general={g*100:.0f}%")
...
def main():
    ...
    if args.dry_run:
        ...
        maritime_mm, _ = load_or_build_cache(
            tokenizer=tokenizer,
            src_jsonl=CPT_CORPUS,
            cache_key="cpt_maritime_4b",
            seq_len=seq_len,
            max_doc_tokens=max_doc_tokens,
        )
        general_mm, _ = load_or_build_cache(
            tokenizer=tokenizer,
            src_jsonl=GENERAL_REPLAY,
            cache_key="cpt_general_4b",
            seq_len=seq_len,
            max_doc_tokens=max_doc_tokens,
        )
    ...
    start = time.time()
    if args.dry_run:
        log_pipeline("PHASE_1A_CPT_4B_DRY_RUN STATUS: TRAINING STARTED")
    else:
        log_pipeline("PHASE_1A_CPT_4B STATUS: TRAINING STARTED")

    # If checkpoints exist, resume automatically to avoid losing long GPU runs.
    resume_path: Path | None = None
    if not args.dry_run:
        resume_env = os.environ.get("CPT_RESUME_FROM_CHECKPOINT", "").strip()
        if resume_env:
            resume_path = Path(resume_env)
        else:
            resume_path = latest_checkpoint_dir(CHECKPOINT_DIR)

        if resume_path is not None and resume_path.exists():
            logger.info(f"Resuming from checkpoint: {resume_path}")
            log_pipeline(f"PHASE_1A_CPT_4B STATUS: RESUMING_FROM_CHECKPOINT {resume_path.name}")
        else:
            resume_path = None

    if resume_path is not None:
        _sanitize_checkpoint_for_transformers_resume(resume_path, phase_tag="PHASE_1A_CPT_4B")
    ...
    if args.dry_run:
        ...
        print("DRY_RUN PASS")
        log_pipeline("PHASE_1A_CPT_4B_DRY_RUN STATUS: PASS")
        return
    ...
    if len(ppl_entries) >= 2:
        ...
        if gate_pass:
            log_pipeline(f"PHASE_1A_CPT_4B STATUS: PASS. Maritime PPL drop {m_drop_pct:.1f}%, general increase {g_increase_pct:.1f}%")
        else:
            log_pipeline(f"PHASE_1A_CPT_4B STATUS: FAIL. Maritime drop {m_drop_pct:.1f}% (need 15%), general increase {g_increase_pct:.1f}% (need ≤10%)")
            sys.exit(1)
    else:
        log_pipeline("PHASE_1A_CPT_4B STATUS: WARNING — insufficient perplexity log entries for gate check")

    log_pipeline(f"PHASE_1A_CPT_4B STATUS: COMPLETE. Model saved to {CHECKPOINT_DIR}/final")
