...
def _sanitize_checkpoint_for_transformers_resume(checkpoint_dir: Path, *, phase_tag: str) -> None:
    """Make a checkpoint safe to resume with recent Transformers + PyTorch.

    Newer Transformers versions call:
        torch.load(rng_state.pth, weights_only=True)

    but the saved RNG state can include numpy objects, which triggers an
    UnpicklingError under the weights-only loader. If that happens, training
    crashes *before* the first step.

    We treat our own checkpoints as trusted and simply disable RNG restore by
    renaming rng_state*.pth so Trainer will skip loading it.
    """
    ...
    for rng_path in rng_files:
        try:
            # Probe whether this file can be loaded in weights-only mode.
...
def main():
    ...
    if not args.dry_run:
        ...
        if resume_path is not None and resume_path.exists():
            ...
            log_pipeline(f"PHASE_1A_CPT_1.7B STATUS: RESUMING_FROM_CHECKPOINT {resume_path.name}")
        else:
            resume_path = None

    if resume_path is not None:
        _sanitize_checkpoint_for_transformers_resume(resume_path, phase_tag="PHASE_1A_CPT_1.7B")
